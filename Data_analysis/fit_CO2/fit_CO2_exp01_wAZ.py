...
def f_dark_1C(t, y, kcf, kcr, fc, fb):
    """
    ...
    #Eqs.S1-S4
    dCe = -k1 * Ce + k2* (H @ Be) + fc * N *(Ci - Ce)/Ve
    dBe = k1 * (G @ Ce) - k2 * Be + fb * N *(Bi - Be)/Ve
...
def sim_dark_1C(T_span, Y0, kcf, kcr, fc, fb, T_eval = None):
    sol_dark = solve_ivp(f_dark_1C,T_span,Y0, method='BDF',args = (kcf, kcr, fc, fb),\
                  t_eval=T_eval)
    return sol_dark
    
def fit_dark_1C(dark_df, Y0):
    """ the Y0 has 7 x 2 = 14 parameters, see f_dark_1C"""
    def f_dark(t_array, kcf, K_cyto, fc, fb):
        kcr = kcf/K_cyto
        s = sim_dark_1C([dark_df.index[0], dark_df.index[-1]], Y0, \
                        kcf, kcr, fc, fb, T_eval = t_array)
        co2s = s.y[:3, :]
        #print(s.y.shape)
        return co2s.flatten()
    #to get kcf, kcr, fc, fb
    low_lims = np.array([10, 9.21, 1.0e-10, 0.0])
    up_lims = np.array([1e3, 92.1, 1.0e-6, 2.0e-11])
    init_guess = np.array([150, 23.2, 2.3e-8, 0.4e-11])
...
f_name = 'Fcyl_3C_S40_exp01_c.csv'
# or your own experimental data file
...
#the two numbers added is when the spike of mass 49 is too high due to slow mixing
...
#measured average effective diameter ~4.35µm
...
ti=t1
for i in range(4):
    dC47[:ti+80] =0#adjust the number added to ti if needed
    if i%2 == 0:
        ti = np.argwhere(dC47==dC47[:700+ti].min())[0][0]
    else:
...
y01 = sol.y[:,-1]*1.206/Ve ##1.206 is the volume before adding cells
y0in = np.zeros(7)
#y0in[:3] = y01[:3]#assumming fast CO2 diffusion to cells
y01 = np.concatenate((y01, y0in))

# for i in range(len(t)-1):
#     plot_df(data.iloc[t[i]:t[i+1]])
#uncomment to see how the t list slices the data

dark0 = data.iloc[t[1]:t[2]-21]#there may be a delay due mixing
plot_df(dark0, True, True)
plot_totalC(dark0)
kcf, Kcyto, fc, fb = fit_dark_1C(dark0, y01)
print('kcf, K_cyto, fc, fb=', kcf, Kcyto, fc, fb)
kcr = kcf/Kcyto
...
#### first light period
light1 = data.iloc[t[2]-21:t[3]-16]
plot_df(light1, True, True)

#dark period following light 1
dark1 = data.iloc[t[3]-16:t[4]-19]
...
mims_df = data.iloc[t0+85:t1-34]#avoiding two ends of the NaHCO3 hydration part
plot_df(mims_df)
mims_do2dt = MIMS_resp(mims_df)#unit in µM/s
# mims_do2dt = 0 #when no clear consumption from mims, assign 0 for calculation

#### calculate the light1 rates

CO2_dark0, HCO3_dark0 = calc_DIC_dark(dark0.iloc[51:],linear= True, window=43)
#two averaged values, linear or not for O2 may have an impact 
#if non-linear region is included

DIC_dark0 = CO2_dark0 + HCO3_dark0
l1_df, Figs_l1 = calc_PS_Urates(light1.iloc[:-8], HCO3_dark0, False, window = 51)
l1_df['cyt_CO2'] = l1_df['smt_CO2'] - l1_df['Uc']/fc*1e9
#1e9 is used to convert mol/cm3 to µM
l1_df.to_csv(f_name[:-14]+'_1st_light_wAZ.csv')

#### calculate light2 rates    
t4_CO2 = ctot.iloc[t[4]-29:t[4]-19].mean()
t4_O2 = data['O2_Ar'].iloc[t[4]-29:t[4]-19].mean()
t2_O2 = data['O2_Ar'].iloc[t[2]-25:t[2]-15].mean()
delta_O2 = t4_O2 - t2_O2 - (data.index[t[4]-19] -data.index[t[2]-15])*mims_do2dt
t4_HCO3 = HCO3_dark0 - delta_O2/1 + CO2_dark0 - t4_CO2#using PQ=1

DIC_t4 = t4_HCO3 + t4_CO2

light2 = data.iloc[t[4]-19:t[5]-19]
plot_df(light2, True, True)

dark2 = data.iloc[t[5]-19:]
plot_df(dark2)

l2_df, Figs = calc_PS_Urates(light2.iloc[:], t4_HCO3, False, window=51)
l2_df['cyt_CO2'] = l2_df['smt_CO2'] - l2_df['Uc']/fc*1e9
#1e9 is used to convert mol/cm3 to µM
l2_df.to_csv(f_name[:-14]+'_2nd_light_wAZ.csv')
...
pdf_name = f_name[:-14]+'_plots_wAZ.pdf'
...
cols = ['Date','S','T/°C','k1','k2','K_blk','ksf','ksr', 'kcf', 'kcr','K_cyt',\
        'fc','fb','fc_BL','fb_BL','fc_M','fb_M','N_total','Vs','Vc','Ve',\
            'AZ','MIMS_do2dt']
paras = [f_name.split()[0][-8:], 40, 3, k1, k2, K_bulk, k1, k2, kcf, kcr, Kcyto,\
         fc, fb, fc_BL, fb_BL, fc_M, fb_M, N, Vs, Vc, Ve,\
             'with', mims_do2dt*Ve/N*1e-9]#no eCA activity so k1, k2 = ksf, ksr

para_s = pd.Series(data = paras, index=cols)
l1_s = pd.concat([para_s, l1])
l2_s = pd.concat([para_s, l2])
smr_wAZ = pd.DataFrame(data=[l1_s,l2_s], columns = l1_s.index)
#one can save the parameters and calculated values from the DataFrame object.